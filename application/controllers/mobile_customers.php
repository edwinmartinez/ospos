...
class Mobile_customers extends Customers 
{
	...
	public function get_customers($customer_id=-1)
	{
		//$config['base_url'] = site_url('?c=mobile_customers&m=index');
		
		if($customer_id != -1) { //return the customer for which id has been specified
	...
	function deleteCustomer($customer_id = -1){
		if($customer_id > -1){
			$this->Customer->delete($customer_id); //<--- comment on debug
			$data['json'] = '{"success":1}';
			$this->load->view('json_view',$data);
		} else {
			$data['json'] = '{"success":0}';
